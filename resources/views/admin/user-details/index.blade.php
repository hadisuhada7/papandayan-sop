...
@section('title', 'Papandayan | User Profiles')
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($user->userDetail))
                                            {{ $user->userDetail->category_user ?? '' }}
                                        @else
                                        @endif
                                    </td>
                                    <td>
                                        @if(isset($user->userDetail) && $user->userDetail->company)
                                            {{ $user->userDetail->company->name }}
                                        @else
                                        @endif
                                    </td>
                                    <!-- <td>
                                        @if(isset($user->userDetail) && $user->userDetail->categories && $user->userDetail->categories->count())
                                            @foreach($user->userDetail->categories as $category)
                                    ...
                                    <td class="text-center">
                                        @if(isset($user->userDetail))
                                            <a href="{{ route('admin.user-details.edit', $user->userDetail) }}" class="btn btn-sm btn-primary item-edit"><i class="fas fa-pencil-alt"></i></a>
                                            <a href="javascript:void(0)" class="btn btn-sm btn-danger item-remove" data-id="{{ $user->userDetail->id }}"><i class="fas fa-trash-alt"></i></a>
                                        @else
                                        @endif
                                    </td>
                                </tr>
        ...
        $(document).ready(function () {
            ...
            $("#datagrid").DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    $('<a href="{{ route('admin.user-details.create') }}" class="btn btn-sm btn-primary" style="margin-left: 10px;">Add New</a>')
            ...
            $("#btn-modal-delete").on("click", function(){
                if (selectedRow) {
                    var deleteUrl = "{{ route('admin.user-details.index') }}/" + selectedRow;
